/* =========================================================================================
   ...
   VERSIÓN: V4.1.0 (Grado Diamante - Lista Blanca VIP en CUSTOM_LIST & Jerarquía Polimórfica JSONB)
...
COMMENT ON COLUMN maint.config.maintenance_action IS 'Acción o módulo de mantenimiento al que aplica esta regla (ALL, VACUUM, VACUUM_FULL, etc.).';
...
-- =========================================================================================
-- 2. TABLA HIJA: Cola Transaccional y Estado de Tareas (Inyección Anti-PID Reuse v2.0)
-- =========================================================================================
...
COMMENT ON COLUMN maint.vacuum_tasks.dead_tuples_pct IS 'Porcentaje de degradación o espacio perdido calculated para ponderar la prioridad.';
...
-- =========================================================================================
-- 3. TABLA DE CONTROL: Reglas y Filtros de Seguridad V4.0.0 (Jerarquía Polimórfica JSONB)
-- =========================================================================================
CREATE TABLE IF NOT EXISTS maint.filters (
    ...
    -- REGLA 2: El candado del Gatekeeper. Solo admite estos 5 valores exactos.
...
COMMENT ON COLUMN maint.filters.action_params IS 'Parámetros JSONB específicos con nombres homologados al orquestador (threshold_pct, min_dead_tuples, force_dead_tuples).';
...
/* =========================================================================================
   PROCEDIMIENTO: maint.sp_orchestrate_vacuum (V4.0.0 - Dynamic Instance Control, User Context & Jerarquía JSONB Homologada)
========================================================================================= */
CREATE OR REPLACE PROCEDURE maint.sp_orchestrate_vacuum(
    ...
    p_keep_history BOOLEAN DEFAULT TRUE         -- TRUE = Conserva auditoría; FALSE = Limpia detalles al terminar
)
LANGUAGE plpgsql AS $$
DECLARE
    v_job_id INT; v_task_id INT; v_schema TEXT; v_table TEXT; v_child_pid INT; v_child_cookie BIGINT;
    ...
    -- [NUEVO V3.6.0]: Intercepción limpia y transmisión de parámetros de contexto 'user'
    v_set_prefix TEXT := '';
    v_param RECORD;
    ...
    -- [NUEVO V3.6.0]: Variable para control dinámico de trabajadores autorizados
    v_max_allowed_workers INT;

    -- Variables para evaluación de umbrales dinámicos homologados V4.0.0
    v_effective_threshold_pct NUMERIC(12,2);
    v_effective_min_dead_tuples INT;
    v_effective_force_dead_tuples INT;
BEGIN
    ...
    FROM maint.config 
    WHERE name = 'max_parallel_vacuum_workers' AND maintenance_action IN ('VACUUM', 'ALL')
    ORDER BY CASE WHEN maintenance_action = 'VACUUM' THEN 1 ELSE 2 END LIMIT 1;
    ...
    -- =====================================================================
    -- [NUEVO V3.6.0] INTERCEPCIÓN DINÁMICA DE PARÁMETROS EN CONTEXTO 'USER'
    -- =====================================================================
    FOR v_param IN (
        SELECT name, setting 
        FROM pg_settings 
    ...
    IF p_verbose THEN
        RAISE INFO '=========================================================';
        RAISE INFO '[DBA SQUAD] INICIANDO ORQUESTADOR VACUUM VANGUARD (V4.1.0 HOMOLOGADO - EXT: %)', COALESCE(v_ext_version, 'v1.x');
    ...
    -- 3. POBLAR COLA DE TAREAS V4.1.0 (TRIAGE CON REGLAS POLIMÓRFICAS JSONB Y SANITIZACIÓN HOMOLOGADA)
    ...
    LEFT JOIN LATERAL (
        SELECT filter_type, action_params 
    ...
    CROSS JOIN LATERAL (
        SELECT 
            -- Extracción Segura de Parámetros Homologados con Protección contra Corrupción
            CASE WHEN mf.filter_type = 'CUSTOM' THEN
                COALESCE(
                    (
                        CASE WHEN (mf.action_params ->> 'threshold_pct') ~ '^-?[0-9]+(\.[0-9]+)?$' 
                             THEN (mf.action_params ->> 'threshold_pct')::NUMERIC 
                             ELSE NULL 
                        END
                    ), 
                    p_threshold_pct
                )
            ELSE p_threshold_pct END AS eff_threshold_pct,

            CASE WHEN mf.filter_type = 'CUSTOM' THEN
                COALESCE(
                    (
                        CASE WHEN (mf.action_params ->> 'min_dead_tuples') ~ '^-?[0-9]+$' 
                             THEN (mf.action_params ->> 'min_dead_tuples')::INT 
                             ELSE NULL 
                        END
                    ), 
                    p_min_dead_tuples
                )
            ELSE p_min_dead_tuples END AS eff_min_dead_tuples,

            CASE WHEN mf.filter_type = 'CUSTOM' THEN
                COALESCE(
                    (
                        CASE WHEN (mf.action_params ->> 'force_dead_tuples') ~ '^-?[0-9]+$' 
                             THEN (mf.action_params ->> 'force_dead_tuples')::INT 
                             ELSE NULL 
                        END
                    ), 
                    p_force_dead_tuples
                )
            ELSE p_force_dead_tuples END AS eff_force_dead_tuples
    ) eval_params
    WHERE st.schemaname <> 'pg_toast' 
      AND st.schemaname <> 'maint'
      ...
      AND (
          (p_scope = 'CUSTOM_LIST' AND (
              mf.filter_type = 'FORCE' OR (
                  mf.filter_type = 'CUSTOM' AND (
                      st.n_dead_tup >= eval_params.eff_min_dead_tuples AND (
                          ((st.n_dead_tup::numeric / NULLIF(st.n_live_tup + st.n_dead_tup, 0)) * 100.0) >= eval_params.eff_threshold_pct
                          OR (eval_params.eff_force_dead_tuples IS NOT NULL AND st.n_dead_tup >= eval_params.eff_force_dead_tuples)
                      )
                  )
              )
          )) OR
          (p_scope LIKE 'SMART%' AND p_scope <> 'CUSTOM_LIST' AND (
              mf.filter_type = 'FORCE' OR ( -- REGLA 2: Fuerza Bruta / Override Total
                  st.n_dead_tup >= eval_params.eff_min_dead_tuples AND ( -- REGLA 3: Evalúa Parámetros Homologados
                      ((st.n_dead_tup::numeric / NULLIF(st.n_live_tup + st.n_dead_tup, 0)) * 100.0) >= eval_params.eff_threshold_pct
                      OR (eval_params.eff_force_dead_tuples IS NOT NULL AND st.n_dead_tup >= eval_params.eff_force_dead_tuples)
                  )
              )
          )) OR
          (p_scope NOT LIKE 'SMART%' AND p_scope <> 'CUSTOM_LIST')
      )
    ...
    LOOP
        -- A. RECOLECTOR FORENSE (Preserva child_pid y child_cookie inmutables para auditoría)
        FOR r_finished IN 
            SELECT task_id, child_pid, child_cookie, schema_name, table_name FROM maint.vacuum_tasks 
        ...
        WHILE v_active_workers < p_parallel_workers AND v_pending_tasks > 0 LOOP
            ...
            IF v_task_id IS NOT NULL THEN
                ...
                -- [NUEVO V3.6.0] Construcción de la sentencia SQL de VACUUM con prefijo dinámico de sesión
                v_raw_sql := v_set_prefix || format('VACUUM (%s) %I.%I;', v_options_str, v_schema, v_table);
