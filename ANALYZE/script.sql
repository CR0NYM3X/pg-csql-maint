...
-- DROP TABLE IF EXISTS mantos.maintenance_jobs CASCADE;
-- TRUNCATE TABLE mantos.maintenance_jobs RESTART IDENTITY CASCADE ;
...
-- DROP TABLE IF EXISTS mantos.mant_analyze_task CASCADE;
-- TRUNCATE TABLE mantos.mant_analyze_task RESTART IDENTITY ;
...
BEGIN
   ...
   PERFORM pg_catalog.set_config('search_path', 'mantos, public, pg_temp', true);
    ...
    -- 1. Crear Job Padre
    INSERT INTO mantos.maintenance_jobs (job_type, maintenance_action, threshold_pct, parallel_workers, status)
    ...
    WHILE v_current_stage <= v_max_stages LOOP
        ...
        IF p_job_type = 'SMART' THEN
            INSERT INTO mantos.mant_analyze_task (job_id, schema_name, table_name, total_filas, filas_afectadas, drift_pct, stage_number)
        ...
        SELECT COUNT(*) INTO v_total_tasks FROM mantos.mant_analyze_task WHERE job_id = v_job_id AND stage_number = v_current_stage;
        ...
        IF v_current_stage = 1 AND v_total_tasks = 0 THEN
            ...
            UPDATE mantos.maintenance_jobs SET status = 'COMPLETED', ended_at = clock_timestamp(), tables_processed = 0 WHERE job_id = v_job_id;
            COMMIT; RETURN;
        END IF;
        ...
        LOOP
            ...
            FOR r_finished IN 
                SELECT task_id, child_pid, schema_name, table_name FROM mantos.mant_analyze_task 
            ...
            LOOP
                BEGIN
                    ...
                    UPDATE mantos.mant_analyze_task SET status = 'SUCCESS', ended_at = clock_timestamp(), child_pid = NULL WHERE task_id = r_finished.task_id;
                ...
                EXCEPTION WHEN OTHERS THEN
                    UPDATE mantos.mant_analyze_task SET status = 'FAILED', ended_at = clock_timestamp(), error_log = SQLERRM, child_pid = NULL WHERE task_id = r_finished.task_id;
            ...
            IF p_cutoff_time IS NOT NULL AND LOCALTIME >= p_cutoff_time THEN
                UPDATE mantos.mant_analyze_task SET status = 'SKIPPED_TIME_LIMIT', error_log = 'Cutoff Time Reached' 
                WHERE job_id = v_job_id AND stage_number = v_current_stage AND status = 'PENDING';
                COMMIT;
            END IF;

            -- C. EVALUACIÓN DE ESTADO
            SELECT COUNT(*) INTO v_active_workers FROM mantos.mant_analyze_task WHERE job_id = v_job_id AND stage_number = v_current_stage AND status = 'RUNNING';
            SELECT COUNT(*) INTO v_pending_tasks FROM mantos.mant_analyze_task WHERE job_id = v_job_id AND stage_number = v_current_stage AND status = 'PENDING';
            ...
            WHILE v_active_workers < p_parallel_workers AND v_pending_tasks > 0 LOOP
                ...
                IF v_task_id IS NOT NULL THEN
                    UPDATE mantos.mant_analyze_task SET status = 'RUNNING', started_at = clock_timestamp() WHERE task_id = v_task_id;
                    COMMIT;
                    ...
                    UPDATE mantos.mant_analyze_task SET child_pid = v_child_pid WHERE task_id = v_task_id;
                    COMMIT;
    ...
    IF EXISTS (SELECT 1 FROM mantos.mant_analyze_task WHERE job_id = v_job_id AND status = 'SKIPPED_TIME_LIMIT') THEN
        UPDATE mantos.maintenance_jobs SET status = 'COMPLETED_WITH_CUTOFF', ended_at = clock_timestamp(), tables_processed = v_success_count WHERE job_id = v_job_id;
    ELSE
        UPDATE mantos.maintenance_jobs SET status = 'COMPLETED', ended_at = clock_timestamp(), tables_processed = v_success_count WHERE job_id = v_job_id;
    END IF;
    COMMIT;
