...
VALUES 
  ('max_parallel_analyze_workers',     '20',    'ANALYZE',     'workers', 'Límite máximo dinámico de workers concurrentes para ANALYZE')
...
COMMENT ON COLUMN maint.config.maintenance_action IS 'Acción o módulo de mantenimiento al que aplica esta regla (ALL, ANALYZE, VACUUM_FULL, etc.).';
...
COMMENT ON COLUMN maint.filters.action_params IS 'Parámetros JSONB específicos con nombres homologados al orquestador (threshold_pct, min_mod_tuples, force_mod_tuples).';
...
/* =========================================================================================
   PROCEDIMIENTO: maint.sp_orchestrate_analyze (V4.0.0 - Dynamic Instance Control & Jerarquía JSONB Homologada Sanitizada)
========================================================================================= */
...
DECLARE
    ...
    v_effective_min_mod_tuples INT;
    v_effective_force_mod_tuples INT;
BEGIN
    ...
    FROM maint.config 
    WHERE name = 'max_parallel_analyze_workers' AND maintenance_action IN ('ANALYZE', 'ALL')
    ORDER BY CASE WHEN maintenance_action = 'ANALYZE' THEN 1 ELSE 2 END LIMIT 1;
    ...
    IF p_verbose THEN
        RAISE INFO '=========================================================';
        RAISE INFO '[DBA SQUAD] INICIANDO ORQUESTADOR ANALYZE VANGUARD (V4.1.0 HOMOLOGADO - EXT: %)', COALESCE(v_ext_version, 'v1.x');
    ...
    WHILE v_current_stage <= v_max_stages LOOP
        ...
        -- POBLAR COLA PARA LA FASE ACTUAL CON V4.1.0 (CORRECCIÓN HERMÉTICA DE SCOPE Y JSONB)
        ...
        CROSS JOIN LATERAL (
            SELECT 
                ...
                CASE WHEN mf.filter_type = 'CUSTOM' THEN
                    COALESCE(
                        (
                            CASE WHEN (mf.action_params ->> 'min_mod_tuples') ~ '^-?[0-9]+$' 
                                 THEN (mf.action_params ->> 'min_mod_tuples')::INT 
                                 ELSE NULL 
                            END
                        ), 
                        p_min_mod_tuples
                    )
                ELSE p_min_mod_tuples END AS eff_min_mod_tuples,

                CASE WHEN mf.filter_type = 'CUSTOM' THEN
                    COALESCE(
                        (
                            CASE WHEN (mf.action_params ->> 'force_mod_tuples') ~ '^-?[0-9]+$' 
                                 THEN (mf.action_params ->> 'force_mod_tuples')::INT 
                                 ELSE NULL 
                            END
                        ), 
                        p_force_mod_tuples
                    )
                ELSE p_force_mod_tuples END AS eff_force_mod_tuples
        ) eval_params
          ...
          AND (
              (p_scope = 'CUSTOM_LIST' AND (
                  mf.filter_type = 'FORCE' OR (
                      mf.filter_type = 'CUSTOM' AND (
                          COALESCE(st.n_mod_since_analyze, 0) >= eval_params.eff_min_mod_tuples AND (
                              ((COALESCE(st.n_mod_since_analyze, 0)::numeric / NULLIF(st.n_live_tup, 0)) * 100.0) >= eval_params.eff_threshold_pct 
                              OR (eval_params.eff_force_mod_tuples IS NOT NULL AND COALESCE(st.n_mod_since_analyze, 0) >= eval_params.eff_force_mod_tuples)
                          )
                      )
                  )
              )) OR
              (p_scope LIKE 'SMART%' AND p_scope <> 'CUSTOM_LIST' AND (
                  mf.filter_type = 'FORCE' OR ( -- REGLA 2: Fuerza Bruta / Override Total
                      COALESCE(st.n_mod_since_analyze, 0) >= eval_params.eff_min_mod_tuples AND ( -- REGLA 3: Evalúa Parámetros Homologados
                          ((COALESCE(st.n_mod_since_analyze, 0)::numeric / NULLIF(st.n_live_tup, 0)) * 100.0) >= eval_params.eff_threshold_pct 
                          OR (eval_params.eff_force_mod_tuples IS NOT NULL AND COALESCE(st.n_mod_since_analyze, 0) >= eval_params.eff_force_mod_tuples)
                      )
                  )
