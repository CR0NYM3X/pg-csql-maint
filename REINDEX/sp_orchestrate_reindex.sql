...
CREATE OR REPLACE PROCEDURE maint.sp_orchestrate_reindex(
    ...
    p_rebuild_invalid BOOLEAN DEFAULT TRUE,     -- Si es TRUE, reconstruye los zombis obligatoriamente
...
BEGIN
    ...
    IF v_total_tasks = 0 THEN
        ...
        IF array_length(v_changed_params, 1) > 0 THEN
            FOR i IN 1 .. array_length(v_changed_params, 1) LOOP EXECUTE format('ALTER ROLE %I RESET %I', current_user, v_changed_params[i]); END LOOP;
        END IF;
    ...
    -- =====================================================================
    -- 5. BUCLE DE DESPACHO ASÍNCRONO Y CHECKSUM
    -- =====================================================================
    LOOP
        ...
        WHILE v_active_workers < p_parallel_workers AND v_pending_tasks > 0 LOOP
            ...
            IF v_task_id IS NOT NULL THEN
                ...
                UPDATE maint.reindex_tasks SET status = 'RUNNING', started_at = clock_timestamp(), old_relfilenode = v_old_node WHERE task_id = v_task_id; COMMIT;
