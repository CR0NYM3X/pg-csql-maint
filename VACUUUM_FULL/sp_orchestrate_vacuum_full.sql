...
-- =========================================================================================
-- 7. ORQUESTADOR QUIRÚRGICO: maint.sp_orchestrate_vacuum_full (V3.4 Con Checksum Físico)
-- =========================================================================================
/* =========================================================================================
...
BEGIN
    ...
    -- =====================================================================
    -- 0. PRE-FLIGHT CHECK: VERIFICACIÓN DE INFRAESTRUCTURA Y ENTORNOS
    -- =====================================================================
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_background') THEN
        RAISE EXCEPTION 'CRÍTICO [INFRAESTRUCTURA]: La extensión "pg_background" no está installeda en la base de datos.';
    END IF;
    ...
    -- =====================================================================
    -- BARRERA DE SEGURIDAD ESTRICTA: FAIL-FAST EN PARALELISMO DE I/O
    -- =====================================================================
    IF p_parallel_workers < 1 OR p_parallel_workers > 2 THEN
        RAISE EXCEPTION 'ALERTA DE SEGURIDAD I/O [RECHAZADO]: Solicitados % hilos para VACUUM FULL. El tope estricto de seguridad permitido es entre 1 y 2 hilos paralelos para evitar colapsar el almacenamiento. Por favor rectifique el parámetro p_parallel_workers.', p_parallel_workers;
    END IF;

    -- =====================================================================
    -- 1. VALIDACIÓN DE GUARDRAILS LÓGICOS
    -- =====================================================================
    ...
    -- =====================================================================
    -- 2. TRIAGE SÍNCRONO DEL RADAR (DATOS CALIENTES DEL DÍA)
    -- =====================================================================
    ...
    IF p_verbose THEN
        RAISE INFO '=========================================================';
        RAISE INFO '[DBA SQUAD] INICIANDO CIRUGIA MAYOR (VACUUM FULL V3.4)';
    ...
    -- REGISTRO DE PARÁMETROS VALIDADOS
    ...
    -- 5. BUCLE DE DESPACHO ASÍNCRONO Y CHECKSUM FÍSICO
    LOOP
        -- A. RECOLECTOR FORENSE CON DOBLE VALIDACIÓN (relfilenode)
        FOR r_finished IN 
            SELECT task_id, child_pid, schema_name, table_name, old_relfilenode 
        ...
        LOOP
            BEGIN
                -- 1. Validación Lógica (Lanza Excepción si falló la instrucción SQL)
