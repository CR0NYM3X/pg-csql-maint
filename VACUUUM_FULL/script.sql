/* =========================================================================================
   ...
   VERSIÓN: V4.0.0 (Grado Diamante - Jerarquía Polimórfica JSONB, relfilenode Checksum & Multi-DB Disk Shield)
...
CREATE TABLE IF NOT EXISTS maint.filters (
    ...
    -- Contenedor Polimórfico de Umbrales Específicos (Reutilizable en cualquier estado)
...
-- Trigger para garantizar auditoría automática e inmutable de actualización
...
DECLARE
    ...
    -- Variables para evaluación de umbrales dinámicos V4.0.0
...
BEGIN
    ...
    IF p_verbose THEN
        RAISE INFO '=========================================================';
        RAISE INFO '[DBA SQUAD] RADAR DE TRIAGE DIARIO (V4.0.0 - LOGIC: % | THRESHOLD: % KB | FORCE: %)', 
                   v_op_upper, (p_bloat_mb_threshold * 1024.0), COALESCE(v_force_bloat_kb::TEXT || ' KB', 'DESACTIVADO');
        RAISE INFO '=========================================================';
    END IF;

    -- Evaluamos TODAS las tablas del ámbito respetando la Jerarquía de Filtros V4.0.0
    FOR r_table IN (
        ...
        BEGIN
            ...
            -- REGLA 2 Y REGLA 3: Evaluación de Jerarquía de Umbrales (FORCE / CUSTOM / HERENCIA GLOBAL)
            IF r_table.filter_type = 'FORCE' THEN
                v_requires_vf := TRUE; -- REGLA 2: Fuerza Bruta / Override Total
            ELSE
                -- REGLA 3: Extracción de umbrales específicos desde action_params JSONB o Herencia del Parámetro Global
                v_effective_bloat_pct_threshold := COALESCE(
                    (r_table.action_params ->> 'max_dead_tuple_pct')::NUMERIC,
                    (r_table.action_params ->> 'bloat_pct_threshold')::NUMERIC,
                    p_bloat_pct_threshold
                );
                
                v_effective_bloat_kb_threshold := COALESCE(
                    ((r_table.action_params ->> 'min_dead_tuples_mb')::NUMERIC * 1024.0),
                    ((r_table.action_params ->> 'bloat_mb_threshold')::NUMERIC * 1024.0),
                    ((r_table.action_params ->> 'bloat_kb_threshold')::NUMERIC),
                    (p_bloat_mb_threshold * 1024.0)
                );
...
-- =========================================================================================
-- 7. ORQUESTADOR QUIRÚRGICO: maint.sp_orchestrate_vacuum_full (V4.0.0 Multi-DB Universal)
-- =========================================================================================
...
BEGIN
    ...
    IF p_verbose THEN
        RAISE INFO '=========================================================';
        RAISE INFO '[DBA SQUAD] INICIANDO CIRUGIA MAYOR (VACUUM FULL V4.0.0 - EXT: %)', COALESCE(v_ext_version, 'v1.x');
    ...
    -- 3. Poblar Cola V4.0.0 (Evaluación de la Jerarquía Polimórfica JSONB)
    FOR r_table IN (
        ...
        ELSE
            -- REGLA 3: Extracción de umbrales específicos desde action_params JSONB o Herencia Global
            v_effective_bloat_pct_threshold := COALESCE(
                (r_table.action_params ->> 'max_dead_tuple_pct')::NUMERIC,
                (r_table.action_params ->> 'bloat_pct_threshold')::NUMERIC,
                p_bloat_pct_threshold
            ...
            IF v_force_bypass THEN
                ...
                v_total_tasks := v_total_tasks + 1;
            ELSIF (
                (v_op_upper = 'AND' AND r_table.total_bloat_pct >= v_effective_bloat_pct_threshold AND r_table.total_bloat_kb >= v_effective_bloat_kb_threshold) OR
                (v_op_upper = 'OR'  AND (r_table.total_bloat_pct >= v_effective_bloat_pct_threshold OR r_table.total_bloat_kb >= v_effective_bloat_kb_threshold))
            ) THEN
